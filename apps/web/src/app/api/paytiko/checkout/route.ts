...
interface PaytikoCheckoutRequest {
...
export async function POST(request: NextRequest) {
    try {
        ...
        let body: PaytikoCheckoutRequest;
        try {
            body = JSON.parse(rawBody) as PaytikoCheckoutRequest;
        } catch {
            return NextResponse.json(
        ...
        if (process.env.PAYTIKO_DEBUG === "1") {
            console.log("[Paytiko] Incoming checkout request", {
                contentType,
        ...
        try {
            ...
            console.log("[Paytiko] Server-calculated prices:", {
                programId,
                accountSize,
                tierId,
                couponCode: couponCode?.trim() || null,
            ...
            if (
                ...
                console.error(
                    "[Paytiko] CRITICAL: Price manipulation detected!",
                    {
                        programId,
                        accountSize,
            ...
            if (
                ...
                console.warn(
                    "[Paytiko] WARNING: Client amount differs from calculated price",
                    {
                        programId,
                        serverCalculated: calculatedPrices.totalPrice,
                        clientSentAmount: clientAmount,
            ...
            console.error("[Paytiko] Error calculating prices:", error);
            logApiError({
                endpoint: "paytiko-checkout-price-calculation",
                error,
                method: "POST",
                statusCode: 500,
                additionalContext: {
        ...
        console.log("[Paytiko Checkout] Cookie extraction:", {
        ...
        if (couponCode?.trim()) {
            try {
                ...
                if (!validationResult.valid) {
                    console.error(
                        "[Paytiko Checkout] Coupon validation failed:",
                        {
                            couponCode,
                ...
                console.error(
                    "[Paytiko Checkout] Error validating coupon:",
                    error,
                );
                logApiError({
                    endpoint: "paytiko-checkout-coupon-validation",
                    error,
                    method: "POST",
                    additionalContext: {
                        couponCode,
        ...
        if (!affiliateUsername && affiliateUsernameFromCookie) {
            affiliateUsername = affiliateUsernameFromCookie;
            console.log(
                "[Paytiko Checkout] Using affiliate username from cookie:",
                affiliateUsername,
            );
        }
        ...
        // Build Paytiko checkout payload
        const timestamp = Math.floor(Date.now() / 1000);
        const paytikoCurrency = (currency || "USD").toUpperCase();
        ...
        const emailRaw = (customerData.email || "").trim();
        // Apply same lowercase logic as signature generation for consistency
        const shouldLowerEmail = ["1", "true", "yes"].includes(
            String(
                process.env.PAYTIKO_LOWERCASE_EMAIL_FOR_SIGNATURE || "",
            ).toLowerCase(),
        );
        const emailForSig = shouldLowerEmail
            ? emailRaw.toLowerCase()
            : emailRaw;
        const signature = generateCheckoutSignature(
            emailForSig,
            timestamp,
            process.env.PAYTIKO_MERCHANT_SECRET || "",
        );
        ...
        const checkoutBody = {
            ...
            lastName: customerData.lastName,
            email: emailForSig,
            currency: paytikoCurrency,
            countryCode,
            orderId: String(purchase.orderNumber || purchase.id),
            lockedAmount: new Decimal(amount).toDecimalPlaces(2).toNumber(),
            phone: customerData.phone,
            city: customerData.city || "",
            ...
            timestamp,
            isPayOut: false,
            cashierDescription: String(purchase.orderNumber || purchase.id),
            useEventBasedRedirects: true,
            ...(process.env.PAYTIKO_MERCHANT_ID
        ...
        if (process.env.PAYTIKO_DEBUG === "1") {
            ...
            console.log("[Paytiko] Outbound checkout body (sanitized)", {
                orderId: checkoutBody.orderId,
                email: maskEmail(checkoutBody.email),
                currency: checkoutBody.currency,
                countryCode: checkoutBody.countryCode,
                lockedAmount: checkoutBody.lockedAmount,
                phone: maskPhone(checkoutBody.phone),
                city: Boolean(checkoutBody.city),
                street: Boolean(checkoutBody.street),
                region: Boolean(checkoutBody.region),
                zipCode: Boolean(checkoutBody.zipCode),
                timestamp: checkoutBody.timestamp,
                signaturePrefix: String(checkoutBody.signature).slice(0, 8),
                signatureEncoding: (
                    process.env.PAYTIKO_SIGNATURE_ENCODING || "hex"
                ).toLowerCase(),
                lowerEmailForSig: ["1", "true", "yes"].includes(
                    String(
                        process.env.PAYTIKO_LOWERCASE_EMAIL_FOR_SIGNATURE || "",
                    ).toLowerCase(),
                ),
        ...
        console.log("[Paytiko] Creating checkout", {
            coreUrl: process.env.PAYTIKO_CORE_URL || "https://core.paytiko.com",
            env: process.env.PAYTIKO_ENV || "PRODUCTION",
            orderId: String(purchase.orderNumber || purchase.id),
            currency: paytikoCurrency,
            countryCode,
            hasPhone: Boolean(customerData.phone),
            ...(process.env.PAYTIKO_DEBUG === "1"
                ? {
                      ts: timestamp,
                      ...
                      lowerEmailForSig: ["1", "true", "yes"].includes(
                          String(
                              process.env
                                  .PAYTIKO_LOWERCASE_EMAIL_FOR_SIGNATURE || "",
                          ).toLowerCase(),
                      ),
                  }
                : {}),
        ...
        const checkout = await createCheckoutSession(checkoutBody);
        if (!checkout.ok) {
            console.error("[Paytiko] Checkout failed", {
                status: checkout.status,
                errorMessage: checkout.error?.errorMessage,
            });
            // Mark purchase failed on error from Paytiko
            try {
                await payload.update({
                    collection: "purchases",
                    id: purchase.id,
                    data: {
                        status: "failed",
                        notes: `Paytiko checkout failed: ${checkout.error?.errorMessage || checkout.status}`,
                        metadata: {
                            ...(purchase.metadata as object),
                            paytikoCheckoutError: checkout.error,
                            paytikoStatusCode: checkout.status,
                ...
                logApiError({
                    endpoint: "paytiko-checkout-mark-failed",
                    error: updateError,
                    method: "POST",
                    additionalContext: {
            ...
            return NextResponse.json(
                {
                    error: "Failed to initialize Paytiko checkout",
                    details: checkout.error?.errorMessage || "Unknown error",
                },
                { status: 502 },
            );
        }
        ...
        try {
            ...
            logApiError({
                endpoint: "paytiko-checkout-persist-mapping",
                error: e,
                method: "POST",
                additionalContext: {
                    purchaseId: purchase.id,
        ...
        try {
            ...
            console.error("[Hyros] Error tracking Paytiko checkout:", error);
            logApiError({
                endpoint: "paytiko-checkout-hyros-tracking",
                error,
                method: "POST",
                additionalContext: {
                    purchaseId: purchase.id,
        ...
        try {
            ...
            console.error("[Klaviyo] Error tracking Paytiko checkout:", error);
            logApiError({
                endpoint: "paytiko-checkout-klaviyo-tracking",
                error,
                method: "POST",
                additionalContext: {
                    purchaseId: purchase.id,
        ...
        return NextResponse.json({
            success: true,
            cashierSessionToken: checkout.data.cashierSessionToken,
            orderId: String(purchase.orderNumber || purchase.id),
            purchase: {
                id: purchase.id,
        ...
        console.error("Error in Paytiko checkout API:", error);
        logApiError({
            endpoint: "paytiko-checkout-main",
            error,
            method: "POST",
            statusCode: 500,
        });
